#!/usr/bin/env python3
"""
Generate pericopes and pages ONLY for chapters that don't already have pericopes files.
Existing pericopes (with media/images) are preserved.
"""
import argparse
import json
import re
...
CHAPTER_DESCRIPTIONS_DE = {
    ...
    8: "Frauen folgen Jesus. Gleichnisse vom Reich. Sturm und Dämonen.",
...
def get_chapter_data(chapter: int, content_dir: Path, language: str) -> Optional[Dict]:
    """Load chapter data from JSON file"""
    json_path = content_dir / f"luke-{chapter}-{language}.json"
...
def generate_pericopes_from_content(content_data: Dict, chapter: int, language: str) -> Dict:
    """Generate pericopes JSON from content data"""
    book_names = {
        'de': 'Lukas',
        'en': 'Luke',
        'pl': 'Łukasz',
...
def generate_astro_page(chapter: int, language: str, description: str) -> str:
    """Generate Astro page content"""
    book_names = {
        'de': ('Lukas', 'Das Evangelium nach Lukas', 'Kapitel', 'Zurück zur Startseite', 'Übersetzung: Aperto Bibel (AB-DE)'),
        'pl': ('Łukasz', 'Ewangelia według Łukasza', 'Rozdział', 'Powrót do strony głównej', 'Tłumaczenie: Aperto Bibel (AB-PL)'),
    }

    book, book_full, chapter_word, back_text, translation_text = book_names.get(language, book_names['de'])

    return f'''---
import BaseLayout from '../../../layouts/BaseLayout.astro';
import PericopeCard from '../../../components/PericopeCard.astro';
import ChapterNav from '../../../components/ChapterNav.astro';
import chapterData from '../../../content/luke-{chapter}-{language}.json';
import pericopes from '../../../content/luke-{chapter}-pericopes-{language}.json';
...
<BaseLayout title="{book} {chapter}" lang="{language}" description="{book_full} {chapter}">
  <header class="chapter-header min-h-[90vh] flex flex-col justify-center items-center text-center px-8 py-24" style="background: linear-gradient(180deg, #FFFBF5 0%, #F5EBE0 100%);">
    <p class="passage-reference mb-6">{book_full}</p>
    <h1 class="headline-mixed text-5xl md:text-7xl lg:text-8xl mb-6">
      <span class="cap">{chapter_word[0]}</span>{chapter_word[1:]} {chapter}
    </h1>
    ...
    <nav class="toc max-w-2xl w-full">
      <ul class="space-y-2">
        {{pericopes.pericopes.map((p) => (
          <li>
            <a href={{`#${{p.id}}`}} class="toc-item flex items-center justify-between py-3 px-4 rounded-lg transition-all hover:bg-white/50">
  ...
  <footer class="chapter-footer py-24 px-8 text-center" style="background: #F5E6DC;">
    <ChapterNav currentChapter={{{chapter}}} lang="{language}" />

    <div class="mt-12">
      <a href={{`${{base}}/{language}/`}} style="background: #8B7355; color: white;" class="inline-flex items-center gap-2 px-8 py-4 rounded-full font-medium transition-all hover:opacity-90">
        ← {back_text}
      </a>
    </div>

    <p class="mt-12 text-sm text-warm-gray">
      {translation_text} | Licencja: CC BY-SA 4.0
    </p>
  </footer>
</BaseLayout>
...
def main():
    parser = argparse.ArgumentParser(description='Generate pericopes and pages for NEW chapters only')
    parser.add_argument('--chapter', type=int, help='Single chapter')
    parser.add_argument('--chapters', help='Chapter range (e.g., 11-24)')
    parser.add_argument('--language', required=True, help='Language code (de, pl)')
    parser.add_argument('--content-dir', default='src/content', help='Content directory')
    parser.add_argument('--pages-dir', default='src/pages', help='Pages directory')
    ...
    print(f"\n📖 Checking {len(chapters)} chapters for {args.language}...\n")

    generated_count = 0
    for chapter in chapters:
        # Check if pericopes file already exists (using correct naming convention)
        ...
        # Load chapter data
        content_data = get_chapter_data(chapter, content_dir, args.language)
        if not content_data:
            print(f"  ❌ Chapter {chapter}: No content file found")
            continue

        # Generate pericopes
        pericopes = generate_pericopes_from_content(content_data, chapter, args.language)

        # Write pericopes file (using unified naming convention)
        pericopes_path = output_dir / f"luke-{chapter}-pericopes-{args.language}.json"
        with open(pericopes_path, 'w', encoding='utf-8') as f:
            json.dump(pericopes, f, ensure_ascii=False, indent=2)

        # Generate page
        descriptions = CHAPTER_DESCRIPTIONS_DE if args.language == 'de' else CHAPTER_DESCRIPTIONS_PL
        description = descriptions.get(chapter, f"Chapter {chapter}")
        page_content = generate_astro_page(chapter, args.language, description)

        page_dir = pages_dir / args.language / 'luke'
        page_dir.mkdir(parents=True, exist_ok=True)
        page_path = page_dir / f"{chapter}.astro"
