...
This is the question that matters most, so we will answer it plainly — including how it changes as we grow.

**In the trial phase we are in now, the system carries each passage as far as it can — to the vision standard described above — and then a human reads it before it is published.** Not a token glance: a review with the authority to change anything, by someone who knows the source and the target language. This is how we learn where the system can be trusted and where it cannot, and it is why we are starting with a small set of chapters rather than rushing out a whole Bible.

**As we move past the trial and scale, that pre-publication read cannot stay the same — and we would argue it should not.** The whole point of Aperto is to reach languages and communities that have waited generations precisely because line-by-line human translation does not scale to them. At that scale, no team can sit in front of every chapter in every language before it appears. So the system is built *not to depend on it*: humans set the method, write the standards, and curate every critic; the automated floor guarantees that nothing broken or anachronistic is ever published; weaker passages appear flagged, not hidden.

**This is where a digital translation has an advantage a printed one never had: it can be living rather than final.** Every Bible translator knows that what ultimately decides whether a rendering works is not the translator's judgment but the audience's reception — whether the words actually land for the people reading them — and that is the one thing no amount of pre-publication review can fully measure beforehand. A printed Bible freezes its best guess for a generation. A digital one does not have to.

**So review becomes a conversation that continues after publishing, not only a gate before it.** Our translation and feedback tool at [translate.aperto.bible](https://translate.aperto.bible) is open for exactly this: read a passage, see the reasoning behind a rendering, and tell us where it rings true and where it does not. That feedback is taken seriously and routed back — a recurring confusion becomes a fix, a register that grates updates a critic's brief, a structural problem updates the method. Parts of that loop are live today; parts are still being wired, and we will keep this page current as that changes.

So when we say elsewhere that human judgment is *multiplied, not replaced*, this is the precise shape of it: humans decide what *good* looks like, review before publishing while we are small enough to, and — at any scale — keep listening to the people the translation is actually for. The machine does the volume. The judgment stays human.
